...
- The sliders are connected to 8 analog pins on an Arduino Nano V3 board.There is for every slider 1 Mutebutton and 1 Status-LED, that shines trough the switch. They're powered from the board's 5V output (see schematic)
- The board connects via a USB C cable to the PC

#### Schematic

![Hardware schematic](scematic.png)
...
- An Arduino Nano, Pro Micro or Uno board
  - I officially recommend using a Nano for their smaller form-factor, friendlier USB connectors and more analog pins. Plus they're cheaper
  - You can also use any other development board that has a Serial over USB interface
- A few slider potentiometers, up to your number of free analog pins (the cheaper ones cost around 1-2 EUR each, and come with a standard 10K Ohm variable resistor. These _should_ work just fine for this project)
...
- Flash the Arduino chip with the sketch in [`arduino\deej-5-sliders-vanilla`](./arduino/deej-5-sliders-vanilla/deej-5-sliders-vanilla.ino)
  - _Important:_ If you have more or less than 8 sliders, you must edit the sketch to match what you have
